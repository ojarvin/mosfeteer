...
export function commonSourceCascade(count = 3, { reactive = false, feedback = false } = {}) {
  ...
  circuit.addComponent('output', { refdes: 'VOUT', x: count * 480, y: -80 });
  ...
  for (let i = 1; i <= count; i++) net(circuit, i === count ? 'VOUT' : `STAGE${i}`, [
    `M${i}.d`, `R${i}.a`, i === count ? 'VOUT.p' : `M${i + 1}.g`, ...(reactive ? [`C${i}.a`] : []),
  ]);
  ...
  if (feedback) {
    ...
    circuit.netOfTerminal('VOUT.p').terminals.push(circuit.resolveTerm('RF.b'));
  }
  return circuit;
}
...
export function cascodeBranches() {
  const circuit = new Circuit();
  circuit.addComponent('input', { refdes: 'VIN', x: -400, y: 0 });
  circuit.addComponent('output', { refdes: 'VOUT', x: 480, y: -240 });
  ...
  net(circuit, 'VOUT', ['M2.d', 'M3.d', 'VOUT.p']);
