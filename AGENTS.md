...
- Desktop launch is `npm run desktop`: Electron uses `src/desktop/main.js`/`preload.cjs`, imports repository circuits once into the OS user-data directory, and stores each circuit plus `circuit.svg` there through `src/desktop/storage.js`; `npm start` remains the HTTP dev/automation path. Desktop support is Linux and macOS only. The native Electron menu is hidden by default; the HTML toolbar remains the workspace control surface, with editing tools in the canvas rail and no separate hotkey strip.

### CLI → server → browser fast loop

`POST /api/circuits/<name>/cmd` accepts `{ "cmd":"<line>" }` or `{ "cmd":"line1\nline2\n..." }`. The server loads the named circuit or starts one, runs `runCommand` for each line, saves `circuits/<name>/circuit.json` and `circuit.svg` if anything mutated, and returns `{ name, mutated, results, state }`. The CLI (`src/cli/index.js`) is a thin HTTP client over the command and deterministic generation endpoints; the generation preview/commit contract is documented in [`docs/circuit-spec.md`](docs/circuit-spec.md). The browser has no natural-language generation tool.
