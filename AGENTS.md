...
- Normal: `i` insert/search, `w` Wire, F3 route mode, `m` connected move, Shift+`m` detached move, `c` repeated copy, `l` multi-point line annotation, `r` rotate, Shift+`r` horizontal mirror, Shift+Up/Down bring selected objects to front/back, Ctrl+`r` vertical mirror, `v` visual, `x` Check, Shift+`x` Save.
