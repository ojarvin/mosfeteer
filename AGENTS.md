...
- Cursor/view: h/j/k and arrows move the cursor where applicable; F fits; D toggles dark mode; `#` grid; C crosshair; `?` help (Escape closes). Ctrl+i toggles italic and Ctrl+b bold on selected labels. `Ctrl+Shift+r` is intentionally unbound.
