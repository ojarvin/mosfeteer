...
- `#` calls `setGrid()`; `#btn-grid` mirrors it. Toolbar buttons have `title` tooltips and distinct inline icons; the active tool is indicated by `aria-pressed` styling. At narrow widths the top toolbar puts File on the first row and style, routing, and view below it; wide layouts keep the groups in one row. SVG z-order is crosshair, bottom annotations, middle wires, top components/labels, pin/junction dots, then selection/net overlays. Layer controls restack selected objects only within their default layer.
